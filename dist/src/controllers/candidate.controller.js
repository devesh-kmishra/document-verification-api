...
export const getCandidateOverview = async (req, res) => {
    ...
    const verificationStatus = getOverallStatusFromEmployments(allStatuses.map((e) => e.status));
    res.json({
...
export const getEmploymentTimeline = async (req, res) => {
    const candidateId = req.params.candidateId;
...
function getOverallStatusFromEmployments(statuses) {
...
function getRiskForStatus(status) {
    switch (status) {
        case "CLEAR":
            return 0;
        case "PENDING":
...
function getOverallStatus(riskScore) {
    if (riskScore <= 20)
        return "CLEAR";
    if (riskScore <= 50)
        return "REVIEW";
