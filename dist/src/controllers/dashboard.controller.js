...
const SLA_DAYS = 7;
export const getVerificationDashboard = async (req, res) => {
    ...
    for (const v of completed) {
        const tatMs = new Date(v.completedAt).getTime() - new Date(v.createdAt).getTime();
    ...
    const averageTatDays = completed.length > 0
        ? Number((totalTatDays / completed.length).toFixed(2))
        : 0;
    const slaComplianceRate = completed.length > 0
